...
def color_thresh(img, rgb_thresh=(160, 160, 160)):
    ...
    #changed
    blank = np.zeros((160,320,3),dtype=np.uint8)
    #variable to numbers
    blank.fill(255)
...
def translate_pix(xpix_rot, ypix_rot, xpos, ypos, scale): 
    ...
    xpix_translated = np.rint(xpos+xpix_rot/scale)
    ypix_translated = np.rint(ypos+ypix_rot/scale)
    # Return the result  
...
# trying to update vision figures
#navitable land
    Rover.vision_image[:,:,0]= 255 * threshed[0];
    Rover.vision_image[:,:,2]= 255 * threshed[1];
    Rover.vision_image[:,:,1]= 255 * threshed[2];
#rock place
#    Rover.vision_image[threshed[1],1] = 200
#    Rover.vision_image[threshed[2],0] = 255
...
#record in the rock pix
    ...
    if len(xpix) != 0:
        ...
        rock=np.zeros((len(xpix),2))
        rock=pix_to_world(xpix, ypix, xpos, ypos, yaw, world_size, scale)
        rock_pix_world = np.rint(np.clip(rock, 0, world_size - 1))
        rock_pix_world=rock_pix_world.astype(int)
        Rover.worldmap[rock_pix_world[1],rock_pix_world[0],1] += 1
    else:
        Rover.samples_insight = 0

    if Rover.samples_insight == 1:
        xpix, ypix = rover_coords(threshed[1])
        distrock, angles = to_polar_coords(xpix, ypix)
        dist, angles = to_polar_coords(xpix, ypix)
        Rover.rock_angle = angles
        Rover.rock_dist = dist
